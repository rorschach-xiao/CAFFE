...
MTCNN::MTCNN(const std::vector<std::string> model_file, const std::vector<std::string> trained_file)
{
    ...
    for(int i = 0; i < model_file.size(); i++)
    {
        ...
        net.reset(new caffe::Net<float>(model_file[i],caffe::TEST));
        net->CopyTrainedLayersFrom(trained_file[i]);
	
		caffe::Blob <float> * input_layer = net->blob_by_name("data").get();
...
{
    std::shared_ptr<caffe::Net<float>> net = nets_[i];
    ...
    caffe::Blob<float>* rect = net->blob_by_name(output_blob_names[0]).get();
    caffe::Blob<float>* confidence = net->blob_by_name(output_blob_names[1]).get();
...
void MTCNN::Predict(const std::vector<cv::Mat> imgs, int i)
{
    ...
    //confidence
    caffe::Blob<float>* confidence = net->blob_by_name(output_blob_names[i]).get();
    ...
    //landmarks
    if( i == 2){
        caffe::Blob<float>* points = net->blob_by_name(output_blob_names[1]).get();
...
{
    caffe::Blob<float>* input_layer = nets_[i]->blob_by_name("data").get();
...
void MTCNN::img_show(cv::Mat img, std::string name)
{
    ...
    cv::imwrite("C:/Users/Ð¤Ñô/caffe-windows/tools/result/" + name + ".jpg", img_show);
    //cv::waitKey(0);
}
